...
const GradeRegistrationView: React.FC<GradeRegistrationViewProps> = ({ 
  ...
  const availableGrades = React.useMemo(() => {
    if (currentUser.role === 'admin' || currentUser.role === 'subdirector' || currentUser.role === 'administrative') {
  ...
  const availableGroupsOrSections = React.useMemo(() => {
    ...
    if (currentUser.role === 'admin' || currentUser.role === 'subdirector' || currentUser.role === 'administrative') {
        ...
        const relevantAssignments = currentUser.assignments?.filter(a => 
            a.grade === selectedGrade && a.subject === selectedSubject
        ) || [];
        ...
        if (isTech) {
            ...
            return relevantAssignments.map(a => ({ value: a.group, label: `Grupo ${a.group}` }));
        }
    }
  ...
  const students = localData.studentsData.filter(s => s.grade === selectedGrade);
  ...
  // Cambiar calificación numérica (Trimestral)
