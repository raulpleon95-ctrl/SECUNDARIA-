...
const Sidebar: React.FC<SidebarProps> = ({ onNavigate, currentPage, onToggle, isCollapsed, user, onLogout }) => {
  ...
  const getNavItems = () => {
    // Roles extendidos que tienen acceso básico
    const staffRoles = ['admin', 'teacher', 'subdirector', 'administrative', 'red_escolar', 'laboratorista', 'apoyo'];
    const adminRoles = ['admin', 'subdirector'];

    const items = [
        { id: 'dashboard', label: 'Dashboard', icon: <Home size={20} />, roles: staffRoles },
        { id: 'grades', label: 'Alumnos y Grupos', icon: <Users size={20} />, roles: ['admin', 'subdirector', 'administrative'] }, 
        { id: 'grade-registration', label: 'Calificaciones', icon: <Book size={20} />, roles: ['admin', 'teacher', 'administrative'] },
        { id: 'reports', label: 'Formatos y Listas', icon: <FileText size={20} />, roles: staffRoles },
        { id: 'teachers', label: 'Personal', icon: <Building2 size={20} />, roles: adminRoles },
        { id: 'schedules', label: 'Horarios', icon: <CalendarDays size={20} />, roles: ['admin', 'subdirector', 'teacher'] },
